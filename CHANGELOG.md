...
- feat(terminal): pane header gains Stop and Clear icon buttons for every non-RDP session — Stop sends Ctrl+C (`\x03`) through the existing `send_session_input` IPC (routed via `sshInput` for SSH panes, disabled while the pane is idle) and Clear sends Ctrl+L (`\x0c`), which PowerShell's PSReadLine binds to the same `Clear-Host` builtin `cls` aliases to, giving a `cls`-equivalent clear that never pollutes shell history or submits text to interactive agents (@the-long-ride)
- feat(workspace): paint every workspace row (root, folder, file, connection, show-more) with the current theme's main background (`--theme-bg`) and move the secondary panel surface to the theme's sidebar fill (`--theme-sidebar-bg`), so workspace items read as filled bands on a distinct sidebar; also fixes the root row referencing the undefined `--theme-sidebar` token, which made it paint transparent (@the-long-ride)
...
- fix(window): round the app window corners properly on Windows 11 — the window is now created transparent and shadowless (`tauri.windows.conf.json` + the detached-terminal builder in `terminal_window.rs`), the page shell paints the theme background inside an 8px CSS radius (`app-window-rounded` via `useWindowRounding`), and the rounded shape is dropped while the window is maximized; previously the OS rounded the frame while the opaque webview content kept square corners, which showed a square corner inside the round window (@the-long-ride)
...
- refactor(terminal): convert the per-pane "Session persistence" control from a native `<select>` to a `w-4 h-4` Save icon button that opens a `role="menu"` popover with `menuitemradio` rows (mirrors the pane header's "Choose session for this pane" picker pattern), so the chrome row stays visually uniform — every affordance is a borderless icon button (@the-long-ride)
