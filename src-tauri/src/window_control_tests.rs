...
#[test]
fn set_fullscreen_accepts_a_mock_window() {
    let _guard = test_support::lock();
    let app = test_support::mock_app();
    let window = mock_window(&app, "fullscreen-window");

    assert!(tauri::async_runtime::block_on(set_fullscreen(window.clone(), true)).is_ok());
    assert!(tauri::async_runtime::block_on(set_fullscreen(window, false)).is_ok());
}
