...
| `invocation_with_completion`<br>`crates/app-core/src/launch.rs` | Expose module behavior | Provide the module’s public/native capability | Execute the implementation in the owning Rust module with explicit inputs/results | When invoked by its caller/IPC flow |
...
| `next_root_order`<br>`crates/app-core/src/workspace_model.rs` | Expose module behavior | Provide the module's public/native capability | Densify the next root-level order value for appended workspaces | When a workspace or import is created |
...
| `save_temp_image`<br>`src-tauri/src/app_utils.rs` | Mutate/persist controlled state | Keep paste artifacts out of renderer-controlled paths | Validate non-empty bytes and write them to a timestamped temp PNG, returning its absolute path | When a user pastes an image into a terminal pane |
...
| `write_tree`<br>`src-tauri/src/connections.rs` | Mutate/persist controlled state | Centralize state changes and side effects | Validate/resolve target then perform the mutation | On a state-changing action |
...
| `begin_shutdown`<br>`src-tauri/src/pty.rs` | Stop sessiond lease retries during app exit | Prevent a teardown race from respawning the daemon | Latch shutdown before Tauri closes the GUI lease | When Tauri requests application exit |
...
| `refresh`<br>`src-tauri/src/pty.rs` | Refresh Tauri-side session metadata from sessiond | Keep restored/detached session PID metadata authoritative before process-sensitive actions | Queries daemon summaries and repopulates the local session cache | When a native action needs current daemon-owned session metadata |
...
| `command_completion_enabled`<br>`src-tauri/src/pty.rs` | Expose module behavior | Provide the module’s public/native capability | Execute the implementation in the owning Rust module with explicit inputs/results | When invoked by its caller/IPC flow |
| `utf8_locale_fallback`<br>`src-tauri/src/pty.rs` | Expose module behavior | Provide the module’s public/native capability | Execute the implementation in the owning Rust module with explicit inputs/results | When invoked by its caller/IPC flow |
...
| `ensure`<br>`src-tauri/src/pty_lease.rs` | Manage the GUI lease lifecycle | Keep sessiond alive only while the app is running and prevent shutdown-time respawn | Start one lease task with bounded retry backoff and stop retrying once Tauri begins exit | During Tauri setup and application shutdown |
| `attach_existing_session`<br>`src-tauri/src/pty.rs` | Attach a renderer/window to an existing live daemon PTY | Reuse the same process during same-app detach/reattach | Subscribe to daemon replay plus live output without spawning a PTY | When restoring a live session or a detached terminal window attaches during the current app run |
| `to_tauri_status`, `send_initial_status`<br>`src-tauri/src/pty_status.rs` | Translate daemon status and initial replay metadata into the Tauri wire status | Keep protocol-to-UI mapping isolated from PTY lifecycle orchestration | Convert daemon status variants and attach snapshots into the shared session status DTO | When a native session stream starts or forwards status |
| `report_detached_terminal_context`<br>`src-tauri/src/terminal_window.rs` | Forward detached pane context | Keep one main-window checkpoint owner while detached windows remain disposable clients | Validate window ownership, deduplicate cwd/title, and emit typed updates to the main window | When a detached shell reports cwd or title changes |
| `interrupt_session`<br>`src-tauri/src/pty_interrupt.rs` | Stop the work currently running in a local PTY without closing the session | Make Stop immediate while leaving the shell ready for another command | Refreshes missing PID metadata, snapshots existing descendants, sends ETX, then terminates only that pre-existing descendant set | When the renderer invokes Stop for a LOCAL terminal |
...
| `write_settings_raw`<br>`src-tauri/src/settings.rs` | Mutate/persist controlled state | Centralize state changes and side effects | Validate/resolve target then perform the mutation | On a state-changing action |
| `export_settings` / `import_settings`<br>`src-tauri/src/settings_transfer.rs` | Expose module behavior | Provide the module's public/native capability | Execute the implementation in the owning Rust module with explicit inputs/results | When invoked by its caller/IPC flow |
| `write_user_theme` / `user_theme_files` / `clear_user_themes`<br>`src-tauri/src/themes.rs` | Mutate/persist controlled state | Centralize state changes and side effects | Validate/resolve target then perform the mutation | On a state-changing action |
...
| `updater_configured`<br>`src-tauri/src/update_manager.rs` | Validate/authorize input or target | Keep OS/process/window behavior in Rust/Tauri | Probe updater plugin state and catch the unconfigured panic as a typed false | Before any native-update command runs |
| `check_for_native_update`, `download_and_install_update`<br>`src-tauri/src/update_manager.rs` | Read/derive runtime or persisted data; initiate a native/runtime action | Provide authoritative data to callers; keep OS/process/window behavior in Rust/Tauri | Check GitHub Releases via tauri-plugin-updater, or download the signed bundle, verify, install, and relaunch | When the renderer asks for an update check or the user accepts an install |
...
| `set_fullscreen`<br>`src-tauri/src/window_control.rs` | Mutate/persist controlled state | Centralize state changes and side effects | Validate/resolve target then perform the mutation | On a state-changing action |
...
| `new_workspace`<br>`src-tauri/src/workspace.rs` | Construct a domain/runtime value | Keep construction rules consistent | Build the typed value from explicit inputs/defaults | When a new value/request is needed |
...
| `import_workspace_file`<br>`src-tauri/src/workspace_lifecycle.rs` | Parse/translate/resolve data | Centralize boundary/transformation semantics | Convert validated inputs into the domain/native representation | When crossing a model/launch/import boundary |
| `remove_workspace`<br>`src-tauri/src/workspace_lifecycle.rs` | Remove/terminate/clean controlled state | Provide explicit lifecycle cleanup | Resolve owned target and remove/terminate it | On cleanup/delete/close |
| `rename_workspace`<br>`src-tauri/src/workspace_lifecycle.rs` | Mutate/persist controlled state | Centralize state changes and side effects | Validate/resolve target then perform the mutation | On a state-changing action |
