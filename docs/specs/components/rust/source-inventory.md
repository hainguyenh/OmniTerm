...
| `attach_existing_session`<br>`src-tauri/src/pty.rs` | Attach another OmniTerm window to an existing live daemon PTY | Reuse the same process during same-app detach/reattach | Subscribe to runtime replay plus live output without spawning a PTY | When a detached terminal window attaches during the current app run |
