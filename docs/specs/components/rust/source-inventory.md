...
| `spawn`<br>`src-tauri/src/pty_output_batch.rs` | Expose module behavior | Provide the module’s public/native capability | Execute the implementation in the owning Rust module with explicit inputs/results | When invoked by its caller/IPC flow |
| `push`<br>`src-tauri/src/pty_output_batch.rs` | Expose module behavior | Provide the module’s public/native capability | Execute the implementation in the owning Rust module with explicit inputs/results | When invoked by its caller/IPC flow |
| `finish`<br>`src-tauri/src/pty_output_batch.rs` | Expose module behavior | Provide the module’s public/native capability | Execute the implementation in the owning Rust module with explicit inputs/results | When invoked by its caller/IPC flow |
...
| `new`<br>`src-tauri/src/session_activity.rs` | Expose module behavior | Provide the module’s public/native capability | Execute the implementation in the owning Rust module with explicit inputs/results | When invoked by its caller/IPC flow |
| `observe`<br>`src-tauri/src/session_activity.rs` | Expose module behavior | Provide the module’s public/native capability | Execute the implementation in the owning Rust module with explicit inputs/results | When invoked by its caller/IPC flow |
| `resolve_tick`<br>`src-tauri/src/session_activity.rs` | Parse/translate/resolve data | Centralize boundary/transformation semantics | Convert validated inputs into the domain/native representation | When crossing a model/launch/import boundary |
| `spawn_poller`<br>`src-tauri/src/session_activity.rs` | Expose module behavior | Provide the module’s public/native capability | Execute the implementation in the owning Rust module with explicit inputs/results | When invoked by its caller/IPC flow |
| `new`<br>`src-tauri/src/session_output.rs` | Expose module behavior | Provide the module’s public/native capability | Execute the implementation in the owning Rust module with explicit inputs/results | When invoked by its caller/IPC flow |
| `push`<br>`src-tauri/src/session_output.rs` | Expose module behavior | Provide the module’s public/native capability | Execute the implementation in the owning Rust module with explicit inputs/results | When invoked by its caller/IPC flow |
| `send_status`<br>`src-tauri/src/session_output.rs` | Mutate/persist controlled state | Centralize state changes and side effects | Validate/resolve target then perform the mutation | On a state-changing action |
| `has_status_sink`<br>`src-tauri/src/session_output.rs` | Expose module behavior | Provide the module’s public/native capability | Execute the implementation in the owning Rust module with explicit inputs/results | When invoked by its caller/IPC flow |
| `busy`<br>`src-tauri/src/session_output.rs` | Expose module behavior | Provide the module’s public/native capability | Execute the implementation in the owning Rust module with explicit inputs/results | When invoked by its caller/IPC flow |
| `detach`<br>`src-tauri/src/session_output.rs` | Initiate a native/runtime action | Keep OS/process/window behavior in Rust/Tauri | Resolve validated inputs then invoke the native/runtime service | On the matching user/runtime action |
| `attach`<br>`src-tauri/src/session_output.rs` | Initiate a native/runtime action | Keep OS/process/window behavior in Rust/Tauri | Resolve validated inputs then invoke the native/runtime service | On the matching user/runtime action |
| `buffered`<br>`src-tauri/src/session_output.rs` | Expose module behavior | Provide the module’s public/native capability | Execute the implementation in the owning Rust module with explicit inputs/results | When invoked by its caller/IPC flow |
| `push_output`<br>`src-tauri/src/session_output.rs` | Mutate/persist controlled state | Centralize state changes and side effects | Validate/resolve target then perform the mutation | On a state-changing action |
| `send_status`<br>`src-tauri/src/session_output.rs` | Mutate/persist controlled state | Centralize state changes and side effects | Validate/resolve target then perform the mutation | On a state-changing action |
...
| `add_workspace_folder`<br>`src-tauri/src/workspace.rs` | Expose module behavior | Provide the module’s public/native capability | Execute the implementation in the owning Rust module with explicit inputs/results | When invoked by its caller/IPC flow |
| `remove_workspace_folder`<br>`src-tauri/src/workspace.rs` | Remove/terminate/clean controlled state | Provide explicit lifecycle cleanup | Resolve owned target and remove/terminate it | On cleanup/delete/close |
